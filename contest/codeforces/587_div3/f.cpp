/**
 * Created by hiramekun at 16:59 on 2019-09-21.
 */
#include <bits/stdc++.h>
...
void solve() {
    ...
    SegmentTree<ll, ll> tree(n + 2, fm, LONG_LONG_MAX);
    vector<P> r2l[200001];
    for (ll i = 0; i < n; ++i) {
        if (s[i] == '1') {
            ll l = max(0LL, i - k), r = min(n - 1, i + k);
            r2l[r].eb(l, i + 1);
        }
    }
    ...
    rep(i, n) {
        // この位置を使った場合
        ll v = tree.query(i, i + 1) + i + 1;

        // それぞれのleftに注目
        // 自分を被覆できる範囲内に存在するものを使った場合
        each(p, r2l[i]) {
            ll l = p.F, c = p.S;
            v = min(v, tree.query(l, i + 1) + c);
        }
        tree.update(i + 1, v);
    }
